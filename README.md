...
- **Adaptive renderer hook** — `src/engine/renderer.ts` keeps the postprocessing-compatible WebGL path as the default and can smoke-test `WebGPURenderer` with `?renderer=webgpu`, falling back to `WebGLRenderer` without changing scene code.
