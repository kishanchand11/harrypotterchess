...
import { files, isLightSquare, positionToSquare, squareToPosition } from '../types/chess';
...
export const ChessBoard: React.FC<ChessBoardProps> = ({
  ...
  return (
    <group>
      ...
      {/* A single transparent input plane makes empty-square clicks reliable even when a piece's
          compound meshes overlap the tile event. The visible slabs remain independent visuals. */}
      <mesh
        position={[0, 0.135, 0]}
        rotation={[-Math.PI / 2, 0, 0]}
        onClick={(event) => {
          event.stopPropagation();
          onSquareClick(positionToSquare(event.point.x, event.point.z));
        }}
        onPointerUp={(event) => {
          event.stopPropagation();
          onSquarePointerUp(positionToSquare(event.point.x, event.point.z));
        }}
        onPointerMove={(event) => onHoverSquare(positionToSquare(event.point.x, event.point.z))}
      >
